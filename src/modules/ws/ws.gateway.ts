		...
		OnGatewayDisconnect<Socket>
{
	...
	afterInit(server: Server): void {
		this.server = server;
		this.logger.log(
			`Gateway WebSocket inicializado em ${envConfig.websocket.path}`,
		);
	}
	...
	async handleConnection(client: Socket): Promise<void> {
		...
		this.logger.log(
			`Conexao WS aceita. socket=${client.id} user=${session.userId}`,
		);
	...
	private requireAuthenticatedSession(
		client: Socket,
	...
	private getJoinedRooms(client: Socket): string[] {
		return [...client.rooms].filter((room) => room !== client.id).sort();
	}
