...
export const envValidationSchema = Joi.object({
	...
	WS_CONNECTION_STATE_RECOVERY_MAX_DISCONNECTION_MS:
		Joi.number().default(120000),
	...
	GOOGLE_CLIENT_ID: Joi.when("GOOGLE_AUTH_ENABLED", {
		...
		then: Joi.string().required().messages({
			...
			"string.empty":
				"GOOGLE_CLIENT_ID cannot be empty when Google Auth is enabled",
		}),
		otherwise: Joi.string().optional().allow(""),
	}),
	...
	BETTER_AUTH_SECRET: Joi.string()
		.min(32)
		.default("change-me-min-32-chars-long-secret-1234567890")
		.messages({
			"string.min": "BETTER_AUTH_SECRET must be at least 32 characters",
		}),
	BETTER_AUTH_URL: Joi.string()
		.uri()
		.default("http://localhost:3000")
		.messages({
			"string.uri": "BETTER_AUTH_URL must be a valid URL",
		}),
