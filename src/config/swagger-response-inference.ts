...
function visitControllerClasses(
	...
	ts.forEachChild(sourceFile, (node) => {
		...
		for (const member of node.members) {
			...
			const summary = getStringPropertyFromDecorator(
				apiDocDecorator,
				"summary",
			);
...
function buildApiResponseSchema(
	...
	const dataType =
		declaredDataType ?? getApiResponseDataType(type, context.checker);
...
function buildSchemaForType(type: ts.Type, context: InferenceContext): any {
	...
	if (
		...
		!context.buildingSchemas.has(schemaName)
	) {
...
function buildEnumSchema(type: ts.Type): any | null {
	...
	if (
		members.length === 0 ||
		members.length !==
			type.types.filter((member) => !isNullableType(member)).length
	) {
		return null;
	}
...
function getApiResponseDataType(
	...
	const declaration =
		dataProperty?.valueDeclaration ?? dataProperty?.declarations?.[0];
...
function getDeclaredApiResponseDataType(
	...
	if (
		!responseTypeNode ||
		!isTypeReferenceNamed(responseTypeNode, "ApiResponseDto")
	) {
		return null;
	}
...
function getTypeArgument(
	...
	const checkerTypeArgument = checker.getTypeArguments(
		type as ts.TypeReference,
	)[index];
	return checkerTypeArgument ?? null;
}

function unwrapPromiseTypeNode(
	typeNode: ts.TypeNode,
): ts.TypeReferenceNode | null {
	if (
		ts.isTypeReferenceNode(typeNode) &&
		isTypeReferenceNamed(typeNode, "Promise")
	) {
...
function isTypeReferenceNamed(
	...
	return (
		ts.isIdentifier(typeNode.typeName) && typeNode.typeName.text === typeName
	);
}
...
function getFirstStringArgument(
	call: ts.CallExpression | ts.Decorator,
): string {
...
function detectResponseHelperKind(
	...
	for (const statement of node.body.statements) {
		...
		const helperKind = getResponseHelperKindFromExpression(
...
function getPropertyDocMetadata(
	declaration: ts.Declaration,
): PropertyDocMetadata {
	if (
		getDecorator(declaration, "NotInResult") ||
		getDecorator(declaration, "ApiHideProperty")
	) {
		return {
...
function getLiteralExpressionValue(expression: ts.Expression): unknown {
	...
	if (ts.isArrayLiteralExpression(expression)) {
		return expression.elements.map((element) =>
			getLiteralExpressionValue(element),
		);
	}
